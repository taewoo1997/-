...
- **플랫폼 선택**: 안드로이드 플랫폼에서 앱을 개발하기로 결정하고, Java를 사용하여 구현을 시작했습니다.
- **서버 개발**: Flask 프레임워크를 사용하여 백엔드 서버를 구축하였으며, 사용자가 촬영한 동영상을 서버로 업로드하고, OCR 및 객체 탐지 결과를 반환하는 기능을 개발했습니다.
- **데이터베이스 설계**: SQLite를 사용하여 도서 정보 및 분석 결과를 저장하기 위한 데이터베이스를 설계하고 구축했습니다.

### 동영상 업로드 기능

- **안드로이드 앱**: 사용자가 도서관에서 스마트폰으로 책장을 촬영한 동영상 파일을 불러올 수 있는 기능을 구현했습니다. 동영상은 책장 단위로 촬영되며, 촬영된 동영상을 서버로 업로드할 수 있습니다.
- **Retrofit2를 이용한 API 통신**: 서버와의 데이터 통신을 위해 Retrofit2 라이브러리를 사용하여 동영상 업로드 및 결과 수신 기능을 구현했습니다.

### OCR 및 객체 탐지

- **네이버 CLOVA OCR API**: 업로드된 동영상에서 추출한 프레임에 대해 CLOVA OCR API를 사용하여 청구기호를 인식하도록 구현했습니다.
- **YOLO v5 모델**: 책의 청구기호를 탐지하기 위해 YOLO v5 모델을 사용하여 객체 탐지 기능을 추가했습니다. 모델 학습은 Google Colab에서 진행했으며, 다양한 서가 배열 상태를 학습시켜 정확한 탐지가 가능하도록 했습니다.

### 순서 오류 탐지 및 결과 반환

- **OpenCV를 활용한 이미지 및 동영상 처리**: 동영상 프레임에서 청구기호를 추출하고, 이를 분석하여 순서가 잘못된 책을 탐지하는 로직을 구현했습니다.
- **결과 시각화**: 분석 결과를 사용자가 이해하기 쉽게 시각화하여 반환하는 기능을 추가했습니다. 이를 통해 사용자는 쉽게 서가의 정렬 오류를 파악할 수 있습니다.

### 클라우드 인프라 배포

- **Oracle Cloud Infrastructure (OCI)**: 서버를 OCI에 배포하여 안정적인 서비스 제공이 가능하도록 했습니다. 서버는 Flask 애플리케이션과 SQLite 데이터베이스로 구성되어 있으며, OCI 환경에서 안정적인 운영이 가능합니다.
